...
## Chat and input overlays

`use-chat-viewport.ts` owns the active chat/input surface, not the global shell.
It retains `100lvh` geometry when there is no keyboard, ignores phantom insets,
and uses the visual viewport while the keyboard is visible. On iOS it fixes the
body and briefly moves a focused text control out of Safari's auto-pan target
before restoring it on the next frame. Touch movement stays within scrollable
content; edge gestures cannot bubble into document panning. It does not disable
multi-touch zoom. All changes are released when the activity is covered or closed.

The optional Modal viewport ref is used only by chat composition and contact
forms. Their inline height takes precedence over the existing standalone modal
patch. Chat composition keeps its input/start controls outside the contact list's
scroll region. Avoid restoring the old parent-rectangle offset compensation: the
focus pan and document scrolling must be handled at their source.
