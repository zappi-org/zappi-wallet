...
- [~] PBKDF2 상향 설계 문서 작성 완료(docs/design/kdf-upgrade.md, 742줄) — 실측 벤치+출처 21건. **정식 이중 리뷰는 에이전트 한도(2026-07-12 리셋)로 미완: 메인이 load-bearing 주장 실코드 검증(전건 일치)으로 대체, 구현은 소유자 결정 대기.** 필수 축 4개 전부 충족 확인. 필수 축 (계획 리뷰 보강):
...
- [x] resetAll 재구성(묶음2b, 리뷰 APPROVED): 동명 reset 6종 고유 개명(last-wins로 debug만 실행되던 잠복 버그 수정) + 60줄 복제 삭제 → 슬라이스 위임. 리뷰 필드 전수 대조: 값 충돌 0, 델타는 전부 구 블록 누락 필드 커버(sync 수신/상환 흔적 6필드 = 드리프트 실증)
- [x] hex/bytes 통일(묶음3d, 리뷰 APPROVED): 어댑터·서비스 층 4파일(support-attachment/encryption.adapter/direct-lnurl/passkey — noble fail-loud 이득, 출력 byte-identical 실증) — core 2곳(nostr-address/security.service)은 층위 순수성으로 의도적 로컬 유지. truncateStr 2벌→utils/format.ts 승격. **상대시간 2벌은 실중복 아님 판정(리뷰 동의) — 미통합 기록**. MainApp //old receive 주석은 묶음2a에서 기삭제
