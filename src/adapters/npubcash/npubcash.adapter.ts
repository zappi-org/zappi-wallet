...
export class NpubcashAdapter implements PaymentAliasProvider {
  ...
  async getAccountInfo(session: AuthSession): Promise<Result<AccountInfo, BaseError>> {
    const result = await this.authFetch<NpubcashResponse<{
      user: { name: string; mintUrl: string; lockQuote: boolean }
    }>>(
      session,
        ...
        ws.onmessage = (event: MessageEvent) => {
          try {
            ...
            if (msg.type === 'challenge') {
              if (!isAllowedChallenge(msg.payload?.url, msg.payload?.method, this.baseUrl, wsPath))
              {
                return
              }
  ...
  private async authFetch<T>(session: AuthSession, url: string, init?: RequestInit): Promise<Result<T, BaseError>> {
    ...
    // A 401 means the cached token was rejected — drop it so the next authenticate() mints a fresh one.
    if (!result.ok && result.error instanceof NpubcashApiError && result.error.statusCode === 401) {
      this.jwtCache.clear()
    }
    return result
  }
...
export function isAllowedChallenge(
  challengeUrl: string, method: string, baseUrl: string, allowedPath: string,
): boolean {
  try {
    const toHttpScheme = (protocol: string): string =>
      ({ 'ws:': 'http:', 'wss:': 'https:'})[protocol] ?? protocol

    const target = new URL(challengeUrl, baseUrl)
    const base = new URL(baseUrl)


    return (
      toHttpScheme(target.protocol) === toHttpScheme(base.protocol) &&
