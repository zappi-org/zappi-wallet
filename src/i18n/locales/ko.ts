export default {
  ...
  settings: {
    ...
    mintChangeFailed: '민트 변경에 실패했습니다',
    recommended: '추천',
  ...
  send: {
    ...
    tokenCreate: {
      ...
      confirmTitle: '생성 확인',
      confirmQuestion: '<b>{{mint}}</b>에서 <b>{{amount}}</b>\n이캐시를 만들까요?',
      reclaimNote: '! 이캐시는 받는 사람이 등록하기 전 까지 되찾을 수 있어요',
      mintLabel: '민트',
      createFee: '생성 수수료',
      postBalance: '생성 후 잔액',
      creating: '생성 중…',
      toggleUnit: '단위 전환 (현재: {{current}})',
      insufficientBalance: '출금 민트 잔고 부족 :',
      changeMint: '민트 변경',
      senderPaysFeeCaption: '받는사람이 금액을 그대로 받을 수 있게 수취 수수료를 이캐시에 추가해요 (선택사항)',
      createdTitle: '이캐시가 만들어졌어요',
      spentTitle: '이캐시가 사용되었어요',
      spentMessage: '{{amount}} 사용되었어요',
      tapToReveal: '탭해서 보기',
      receiveFeeAmount: '수취 수수료 {{amount}}',
      share: '공유',
      reclaim: '되찾기',
      reclaiming: '되찾는 중…',
    ...
    confirm: {
      ...
      fullRequestQuestion: '<b>{{mint}}</b>에서\n{{amount}}을\n보낼까요?',
