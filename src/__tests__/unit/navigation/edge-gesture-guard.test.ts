/**
 * edge-gesture-guard tests — the OS edge-swipe suppressor that also re-synthesizes the click
 * a preventDefaulted edge tap would otherwise swallow. IS_IOS_LIKE is a module-load const, so
 * the iPhone UA is stamped via vi.hoisted() before the module graph evaluates; standalone is
 * spoofed per test so isEdgeGuardActive() is true and the guard actually installs.
 */
...
describe('edge-gesture-guard', () => {
  ...
  beforeEach(() => {
    ...
    navigateToScreen('settings', { reset: true }) // pushed screen → stack depth 2 (guard armed)
  ...
  it('claims the left edge on a pushed screen (preventDefault)', () => {
  ...
  it('is inert at the stack root — an edge tap on Home is left to the page', () => {
    navigateToScreen('home', { reset: true }) // stack depth back to 1
    const ev = touchEvent('touchstart', true, EDGE_TOUCH)
    window.dispatchEvent(ev)
    window.dispatchEvent(touchEvent('touchend', true, EDGE_TOUCH))
