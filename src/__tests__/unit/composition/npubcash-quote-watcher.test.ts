...
describe('NpubcashQuoteWatcher', () => {
  ...
  describe('pending quote independent retry', () => {
    // Advance fake timers in steps until cond() holds — waits out the
    // exponential backoff without hardcoding its timeline. Bounded budget so
    // a broken retry loop fails instead of hanging the test.
    const advanceUntil = async (cond: () => boolean, maxMs = 120_000) => {
      let elapsed = 0
    ...
    // Backoff timeline (fake timers): first pass burns handleQuote's in-call
    // retries (0.5+1+2+4s), saves pending at t≈7.5s. Sync retries then fire at
    // 2s/4s/8s/16s intervals; the 30s pending backoff (RETRY.INITIAL_DELAY)
    // becomes due at t≈37.5s.
    const setupFailingMint = (h: ReturnType<typeof createHarness>) => {
      h.provider.subscribePaidQuotes.mockResolvedValue(Ok(vi.fn()))
      h.provider.getPaidQuotes
        .mockResolvedValueOnce(Ok([quote({ quoteId: 'q1', paidAt: 1_000 })]))
        .mockResolvedValue(Ok([]))
      h.mint.mintAndReceive.mockRejectedValue(new Error('mint down'))
    }

    it('retries a failed mint on its own backoff without any new WS notification', async () => {
      vi.useFakeTimers()
      const h = createHarness()
      setupFailingMint(h)

      await h.watcher.start()
      await advanceUntil(() => h.pendingQuotesRepo.save.mock.calls.length > 0)
      expect(h.mint.mintAndReceive).toHaveBeenCalledTimes(5)
      expect(h.pendingQuotesRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ quoteId: 'q1', state: 'retry' }),
      )

      // HTTP succeeded — the retry loop must keep itself alive on its own.
      // (5 initial attempts + 5 from the first due drain.)
      await advanceUntil(() => h.mint.mintAndReceive.mock.calls.length >= 10)
      expect(h.mint.mintAndReceive).toHaveBeenCalledTimes(10)
    })

    it('stops the retry loop and settles once the mint recovers', async () => {
      vi.useFakeTimers()
      const h = createHarness()
      h.provider.subscribePaidQuotes.mockResolvedValue(Ok(vi.fn()))
      h.provider.getPaidQuotes
      ...
      await advanceUntil(() => h.pendingQuotesRepo.delete.mock.calls.length > 0)
      expect(h.mint.mintAndReceive).toHaveBeenCalledTimes(6)
      expect(h.eventBus.emit).toHaveBeenCalledTimes(1)

      // Loop is done — no further retries.
      await vi.advanceTimersByTimeAsync(60_000)
      expect(h.mint.mintAndReceive).toHaveBeenCalledTimes(6)
    })

    it('stop() cancels the scheduled pending retry', async () => {
      vi.useFakeTimers()
      const h = createHarness()
      setupFailingMint(h)
