...
import { readMyAddressCache, writeMyAddressCache, type MyAddressCache } from '@/ui/utils/my-address-cache'
...
function useDepositMint(
  ...
  // Per-account entry: a fresh account must never inherit the previous
  // account's cached address, even when the server lookup fails.
    ...
    fetchDeposit
      .then((result) => {
        ...
        if (result.ok) {
          ...
          if (nostrPubkey) {
            writeMyAddressCache(nostrPubkey, next)
            setCache(next)
          }
          setDeposit(
  ...
  const handleRegister = useCallback(async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        if (nostrPubkey) writeMyAddressCache(nostrPubkey, { address: registered, updatedAt: Date.now() })
        await onSaveSettings?.({
          lightningAddress: registered,
          npubcashUrl: NPUBCASH_URL,
        })
  ...
  return (
    <div className="h-full bg-background text-foreground flex flex-col pt-safe">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col items-center px-6 pt-4 pb-app">
        ...
        <div className="relative w-full max-w-qr">
            ...
            <div role="tablist" className="flex px-6 pt-2">
              {TABS.map((key) => (
                <button
                  ...
                  className={`flex-1 border-b-2 pb-2 pt-3 text-caption transition-colors ${
                    tab === key
                ...
                lightningComingSoon ? null : !displayAddress ? null : (
                  <>
                    ...
                    <div className="mt-[23px]">
                      <p className="text-caption text-foreground-muted">{t('myAddress.addressLabel')}</p>
                      <div className="mt-1 flex items-center justify-between gap-3">
                        ...
                        <button
                          ...
                          className="flex h-[26px] w-[50px] shrink-0 items-center justify-center gap-1 rounded-[7px] border border-neutral-300 bg-background-card text-[11px] text-foreground active:scale-95 motion-reduce:active:scale-100 transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40"
                        >
                    ...
                    <div className="mt-5">
                      <p className="text-caption text-foreground-muted">{t('myAddress.receiveMint')}</p>
                      {mintUrl ? (
                        <div className="mt-2 flex items-center gap-2">
                          <MintIcon
                            ...
                            className="h-4.5 w-4.5 bg-neutral-200"
                          />
                          <span className="min-w-0 truncate text-body font-medium">{getDisplayName(mintUrl)}</span>
                          <button
                            type="button"
                            onClick={() => { hapticTap(); setMintPickerOpen(true) }}
                            className="ml-1 flex h-[26px] w-[50px] shrink-0 items-center justify-center gap-1 rounded-[7px] border border-neutral-300 bg-background-card text-[11px] text-foreground active:scale-95 motion-reduce:active:scale-100 transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40"
                          >
        ...
        {value && (
          <div className="mt-5 flex justify-center gap-8">
            <button
              onClick={handleCopy}
              className="flex min-h-9 items-center gap-1.5 rounded-[7px] border border-neutral-200 bg-background-card px-4 text-caption font-medium text-foreground active:scale-95 motion-reduce:active:scale-100 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40"
            >
