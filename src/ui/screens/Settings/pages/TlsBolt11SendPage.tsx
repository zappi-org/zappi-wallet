...
import { ArrowLeft, Zap, RotateCcw, ChevronRight } from 'lucide-react'
...
interface TlsBolt11SendPageProps {
  onBack: () => void
}

export function TlsBolt11SendPage({ onBack }: TlsBolt11SendPageProps) {
  ...
  const handleSend = async () => {
    ...
    if (!invoice.trim()) {
      addToast({ type: 'error', message: 'Invoice required' })
      return
    }
    if (!amount.trim()) {
      ...
      const transfer = await registry.transferLifecycle.initiateTransfer(
        {
          ...
          recipient: invoice.trim(),
          memo: memo.trim() || undefined,
        },
        'bolt11',
      )
      setLastResult(`Created: ${transfer.id}\nPhase: ${transfer.phase}`)
      addToast({ type: 'success', message: `TLS transfer ${transfer.phase}` })
    } catch (err) {
    ...
    <div className="h-full bg-background text-foreground flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6 space-y-6">
        ...
        <div className="bg-background-card rounded-card p-4 space-y-3">
          ...
          <div>
            <label className="text-caption text-foreground-muted block mb-1">Invoice (bolt11)</label>
            <textarea
              value={invoice}
              onChange={(e) => setInvoice(e.target.value)}
              rows={3}
              className="w-full bg-background text-foreground rounded-lg px-3 py-2 text-body border border-foreground/10 focus:outline-none focus:ring-2 focus:ring-accent resize-none"
              placeholder="lnbc..."
            />
          </div>
          <div>
