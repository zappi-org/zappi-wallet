...
import { ArrowLeft, MessageSquare, ClipboardCopy, RotateCcw, ChevronRight } from 'lucide-react'
import { useServiceRegistry } from '@/ui/hooks/use-service-registry'
import { useAppStore } from '@/store'
import { Button } from '@/ui/components/common/Button'
import { MintSelectBottomSheet } from '@/ui/components/payment/MintSelectBottomSheet'

interface TlsCreqPageProps {
  onBack: () => void
}

export function TlsCreqPage({ onBack }: TlsCreqPageProps) {
  ...
  const handleCreate = async () => {
    ...
    if (!nostrPubkey) {
      addToast({ type: 'error', message: 'Nostr pubkey not available' })
      return
    }
    if (!settings.relays?.length) {
      addToast({ type: 'error', message: 'No Nostr relays configured' })
      return
    }
    ...
    try {
      const nprofile = registry.crypto.encodeNprofile(nostrPubkey, settings.relays)
      const result = registry.paymentRequest.createNostrPaymentRequest({
        amount: Number(amount),
        unit: 'sat',
        mints: [selectedMintUrl],
        nostrTarget: nprofile,
        description: description.trim() || undefined,
      })

      setCreqString(result.request)
      setRequestId(result.id)
      setLastResult(`Request ID: ${result.id}`)
      addToast({ type: 'success', message: 'Creq created' })
    } catch (err) {
  ...
  const copyCreq = () => {
    ...
    void navigator.clipboard.writeText(creqString)
    addToast({ type: 'success', message: 'Creq copied to clipboard' })
  }
  ...
  // Show all incoming pending transfers that might be related to creq
        ...
        {creqString && (
          <div className="bg-background-card rounded-card p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-caption text-foreground-muted">Creq (copy & share)</p>
              <button
                onClick={copyCreq}
            ...
            <p className="text-body break-all font-mono text-sm">{creqString}</p>
            <p className="text-label text-foreground-muted">Request ID: {requestId}</p>
          </div>
        )}
