...
export class PaymentAliasService implements PaymentAliasUseCase {
  constructor(
    ...
    private readonly codec: Pick<TokenCodec, 'decodePaymentRequest'>,
  ) {}
  ...
  async getCurrentAlias(privkey: string): Promise<Result<AliasResult, BaseError>> {
    const signer = this.createSigner(privkey)
  ...
  async checkAliasPrice(privkey: string, alias: string): Promise<Result<AliasPriceInfo, BaseError>> {
    ...
    if (!result.ok && result.error instanceof NpubcashPaymentRequiredError) {
      const parsed = this.codec.decodePaymentRequest(result.error.encodedRequest)
  ...
  async changeAlias(privkey: string, alias: string, cashuToken: string, memo?: string): Promise<Result<AliasResult, BaseError>> {
    ...
    if (!result.ok && result.error instanceof NpubcashPaymentRequiredError && !cashuToken) {
      ...
      const parsed = this.codec.decodePaymentRequest(paymentReq.encodedRequest)
      const mintUrl = parsed.mints[0]
