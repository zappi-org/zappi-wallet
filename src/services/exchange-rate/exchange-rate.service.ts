import { ExchangeRateRepository } from '@/data/repositories/exchange-rate.repository'
...
class ExchangeRateService {
  private repository = new ExchangeRateRepository()
  ...
  private async doFetch(): Promise<Record<string, number> | null> {
    try {
      ...
      const now = Date.now()
      await this.repository.save(rates, now)
